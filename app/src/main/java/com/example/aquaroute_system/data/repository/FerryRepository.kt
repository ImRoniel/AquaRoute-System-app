...
class FerryRepository(private val firestore: FirebaseFirestore) {
    ...
    suspend fun getAllFerries(): List<Ferry> {
        ...
        return try {
            ...
            snapshot.documents.mapNotNull { doc -> parseFerryDocument(doc) }
        } catch (e: Exception) {
    ...
    /**
     * Observe all ferries in real-time
     */
    fun observeAllFerries(): Flow<Result<List<Ferry>>> = kotlinx.coroutines.flow.callbackFlow {
        val listener = firestore.collection(FERRIES_COLLECTION)
            .addSnapshotListener { snapshot, error ->
                ...
                if (snapshot != null) {
                    val ferries = snapshot.documents.mapNotNull { doc -> parseFerryDocument(doc) }
                    trySend(Result.Success(ferries))
                }
            }
        
        awaitClose { 
            Log.d(TAG, "Closing ferry real-time listener")
            listener.remove() 
        }
    }

    /**
     * Helper to parse a Firestore document into a Ferry model
     */
    private fun parseFerryDocument(doc: com.google.firebase.firestore.DocumentSnapshot): Ferry? {
        return try {
